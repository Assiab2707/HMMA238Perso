...
```python
s0= 'Toto said: "Hello! How\'s it going?"'
print(s0)
```
...
```python
len(s) #longueur chaine de caractères
s[0]  # first character
```
...
```python
s[-1]  # last character
s[len(s)-1] #Equivelent à ligne du dessus (attention -1 car on commence l'indexation à 0)
```

```python
s[:5] #5 premiers caractères 
s[1:5] #caractères 1 à 5 
s[:]#Ensemble des caractères 
s[5:]#Du 5 eme caractères à la fin
```
```python
index =6 
print(s[:index]) #5 premiers caractères 
print(s[1:index]) #caractères 1 à 5 
print(s[:])#Ensemble des caractères 
print(s[index:])#Du index caractères à la fin
...
#Sans boucle
print("{0:1.1f}".format(e))
print("{0:1.10f}".format(e))
print("{0:1.20f}".format(e))
print("{0:1.50f}".format(e))
